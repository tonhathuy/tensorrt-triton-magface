...
if __name__ == '__main__':
    ...
    if FLAGS.mode == 'dummy':
        ...
        # inputs.append(grpcclient.InferInput('input', [1, 3, FLAGS.width, FLAGS.height], "FP32"))
        # inputs[1].set_data_from_numpy(np.ones(shape=(1, 3, FLAGS.width, FLAGS.height), dtype=np.float32))
    ...
    if FLAGS.mode == 'image':
        ...
        if input_image is None:
            ...
            sys.exit(1)
        img,orgimg=preprocess(input_image, FLAGS.width, 32) 
        inputs[0].set_data_from_numpy(img.numpy())
        ...
        if FLAGS.model_info:
            ...
            print(statistics)
        print("Done")
        print('time infer:', time.time() - time_start)
        result = results.as_numpy('output')
        print(f"Received result buffer of size {result.shape}")
        print(f"Naive buffer sum: {np.sum(result)}")
